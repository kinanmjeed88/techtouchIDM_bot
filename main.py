...
async def get_reply_text_and_save(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await show_replies_menu(update, context, from_conversation=True) # العودة لقائمة الردود
...
async def show_replies_menu(update: Update, context: ContextTypes.DEFAULT_TYPE, from_conversation=False):
    ...
    reply_markup = InlineKeyboardMarkup(keyboard)
    await edit_func("إدارة الردود التلقائية:", reply_markup=reply_markup)
...
async def view_delete_replies(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for reply in replies:
        # زر الحذف يرسل `delete_` متبوعاً بالكلمة المفتاحية
...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == \'get_analysis_report\':
        # وظيفة تقرير التحليل
        await query.message.edit_text("جاري إعداد التقرير...")
        db = SessionLocal()
        try:
            ...
            if total_messages == 0:
                await query.message.edit_text("لا توجد رسائل لتحليلها في آخر 7 أيام.")
                return
        ...
        if delete_reply(keyword_to_delete):
            ...
            await view_delete_replies(update, context) # تحديث القائمة
        else:
...
def main() -> None:
    ...
    application.add_handler(MessageHandler(filters.TEXT & filters.Regex(r\'\\^يمان\\$\'
), show_control_panel))
    application.add_handler(CallbackQueryHandler(button_handler))
    application.add_handler(MessageHandler(filters.TEXT & ~filters.COMMAND, process_group_message))
    # ملاحظة: يجب وضع معالج الرسائل العام في النهاية
